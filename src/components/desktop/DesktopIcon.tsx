...
export function DesktopIcon({id,label,icon,onOpen}:{id:string;label:string;icon:XpIconName;onOpen:()=>void}){const selected=useDesktopStore(s=>s.selectedIconId===id);const select=useDesktopStore(s=>s.selectIcon);const last=useRef(0);return <button className={`desktop-icon ${selected?'selected':''}`} onClick={()=>{const now=Date.now();select(id);if(now-last.current<430)onOpen();last.current=now}} aria-label={`Open ${label}`} data-testid={`desktop-${id}`}><XpIcon name={icon}/><span>{label}</span></button>}
